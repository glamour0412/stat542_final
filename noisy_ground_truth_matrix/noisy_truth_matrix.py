...
# the idea it to create a 200*200 matrix with roughly 50% sparsity, based on the assumption that 50% of the survey entry is noisy.
...
# use 'print(filtered_data.count)' to make sure the sparsity is roughly 50%
...
# create df_fill3 to replace all the NaN as 3, based on the assumption that classmate will rate unfamiliar restaurant 3
df_fill3 = df.fillna(3)
df_fill3.to_csv(os.path.join("noisy_truth_matrix", f"df_fill3.csv"))

# create df_c to replace all the NaN as user's mean rating minus c, with c values 0.1, 0.2, 0.3, 0.5, 0.75, 1
